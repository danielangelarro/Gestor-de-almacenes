...
export default {
    name: 'proveedor-input-sugerence',
    props: {
        value_input: {
            type: String,
            default: ''
    ...
    computed: {
        filtered() {
            let results = this.suggestions.filter(suggestion => {
                return (suggestion.nombres + " " + suggestion.apellidos).toLowerCase().indexOf(this.input.toLowerCase()) > -1;
            });
    ...
    methods: {
        ...
        select(suggestion) {
            this.input = suggestion.nombres + ' ' + suggestion.apellidos;
            this.value = suggestion.iD_Proveedor;
            this.open = false;
        ...
        async getProveedores() {
            axios.get(`${API_URL}/proveedor`)
                .then(res => {
                    this.suggestions = res.data.value['proveedors'];
                  })
                  .catch(error => {
