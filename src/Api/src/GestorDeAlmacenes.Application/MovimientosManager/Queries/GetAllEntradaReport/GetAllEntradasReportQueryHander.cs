...
public class GetAllEntradasReportQueryHandler : IRequestHandler<GetAllEntradasReportQuery, ErrorOr<Document>>
{
    private readonly IMovimientoRepository _repository;

    public GetAllEntradasReportQueryHandler(IMovimientoRepository entradaoRepository)
    {
        _repository = entradaoRepository;
    }

    public async Task<ErrorOr<Document>> Handle(GetAllEntradasReportQuery query, CancellationToken cancellationToken)
    {
        ...
        headerRow.Cells.Add("FECHA DE ENTRADA");
        ...
        foreach (var entrada in entradas)
        {
            ...
            row.Cells.Add(entrada.Producto_Name);
            row.Cells.Add(entrada.Proveedor_Name);
            row.Cells.Add(entrada.Autorization);
            row.Cells.Add(entrada.Cantidad.ToString());
            row.Cells.Add("$" + entrada.Precio_Unidad.ToString());
            row.Cells.Add("$" + (entrada.Precio_Unidad * entrada.Cantidad).ToString());
            row.Cells.Add(entrada.Fecha.ToShortDateString());
            row.Cells.Add(entrada.Fecha_Caducidad.ToShortDateString());
        }

        Page page = document.Pages.Add();

        // Agregar el subtítulo con el nombre del documento
        TextFragment title = new TextFragment("Entradas de productos");
